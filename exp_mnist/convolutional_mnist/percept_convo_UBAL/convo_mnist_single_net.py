...
from exp_mnist.convolutional_mnist.percept_convo_UBAL.mnist_data_provider import MNISTData
from pathlib import Path

from exp_mnist.convolutional_mnist.percept_convo_UBAL.percep_mnist_UBAL import UBALSim

target_type = 'perceptual'
sigmoid = torch.nn.Sigmoid()
softmax = torch.nn.Softmax(dim=1)
relu = torch.nn.ReLU()
act_fun_F = [relu, relu, relu, relu]
act_fun_B = [relu, relu, relu, relu]

settings_conv = [{"in_channels":1, "out_channels":1, "kernel_size":6, "stride":2, "padding":1},
                {"in_channels":1, "out_channels":1, "kernel_size":5, "stride":1, "padding":0},
                {"in_channels":1, "out_channels":1, "kernel_size":5, "stride":1, "padding":0}]
settings_trans = [{"in_channels":1, "out_channels":1, "kernel_size":6, "stride":2, "padding":1},
                {"in_channels":1, "out_channels":1, "kernel_size":5, "stride":1, "padding":0},
                {"in_channels":1, "out_channels":1, "kernel_size":5, "stride":1, "padding":0}]
learning_rate = 0.01
init_w_mean = 0.0
...
gammasF = [float("nan"), 0.5, 0.5, 0.5]
gammasB = [0.5, 0.5, 0.5, float("nan")]
# betas = [0.0, 1.0, 0.0]
...
result_path = str(Path(__file__).parent.parent.parent.parent) + "/results/mnist/"
