...
class Perceptual:
    def __call__(self, x):
        with open("perceptual_labels.py") as f:
            percep_labels = f.read()
            percep_labels = ast.literal_eval(percep_labels)
            label = torch.FloatTensor(percep_labels[str(x)]).flatten()
        return label
...
class MNISTData:
    ...
    def __init__(self, train_size=55000, validation_size=5000,
        ...
        self.validation_images = torch.stack(validation_images).view(-1, self.image_size)
        ...
        self.test_images = torch.stack(test_images_list).view(-1, self.image_size)
    ...
    def percep_mnist_labels(self):
        ...
        for i in percep_labels:
            tuple += (torch.FloatTensor(percep_labels[str(i)]).flatten(), )
